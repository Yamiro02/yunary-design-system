...
| **Carte d'état héros** (`StateCard` de la v1 : cadre plein, pastille `heros` ronde `brand`/`danger`, titre `subheading`, corps muted, appoint — l'attente, l'indisponible, l'erreur, le cas limite ; `EmptyState` est un état vide en pointillés, pas un héros) | shell (audit d'onboarding, Creator › Profil) | `AuditStateCard` composé aux jetons dans `src/audit/AuditStateCard.tsx` | une `StateCard` du socle, ou une variante `hero`/`solid` d'`EmptyState` |
